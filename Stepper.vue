<template>
  <div class="flex justify-center items-center w-full" v-bind="$attrs">
    <div
      :class="`w-full max-w-xl p-8 rounded-[2rem] shadow-[0_20px_25px_-5px_rgba(0,0,0,0.1),0_10px_10px_-5px_rgba(0,0,0,0.04)] ${stepCircleContainerClassName}`"
        ...
        <template v-for="(_, index) in stepsArray" :key="index + 1">
          ...
          >
            <Motion
              ...
              :animate="
                currentStep > index + 1
                  ? { width: '100%', backgroundColor: '#9fe88d' }
...
const getStepIndicatorStyle = (step: number) => {
  ...
  switch (status) {
    case 'active':
    case 'complete':
      return { backgroundColor: '#9fe88d', color: '#fff' };
    default:
